...
export default {
  ...
  methods: {
    ...
    onLike(index, like) {
      this.$set(this.nftList[index], 'like', like, false)
    },
    getOwners(nfts){
        ...
        for(let j = 0; j < nfts.length; j++){
          ...
          for(let z = 0; z < _nft.itemList.length; z++){
            ...
            owners.push({
              ...
              sellToken: nft.nft.address,
              sellTokenId: nft.nft.tokenId
            });
          }
          nft.owners = owners;
          break;
    ...
    queryMedia(nfts){
      ...
      _nfts = this.$tools.serializeNfts(nfts);
      let data={
        info:_nfts,
        }
      if(data.info.length){
      ...
      for (var i = 0; i < this.nftList.length; i++) {
        ...
        for (var l = 0; l < likes.length; l++) {
          ...
          if (nft.nft.address == like[0] && nft.nft.tokenId == like[1]) {
            ...
            // this.$set(this.nftList[i], 'like', 'true')  
          }
        }
      }
      this.nftList = JSON.parse(JSON.stringify(this.nftList));
