<template>
  <uni-popup  class="popupClass" ref='cancel_sale' type="bottom"  >
    ...
    <view class="clubart-dialog-body" v-if="!confirm">
      ...
      <view class="process"  >
        ...
        <view class="process-btn">
          <button class="nft-dialog-btn" size="mini" @click="onSubmit" type="success" :loading="confirm">{{$t('dialog.confirm')}}</button>
        </view>
        <view class="process-btn">
...
export default {
  ...
  methods: {
    ...
    async cancelSale(){
      ...
      buyValue = buyValue.multipliedBy(new BigNumber(10).exponentiatedBy(this.order.payToken.decimals))
      var order = {
      ...
      return new Promise((resolve, reject) => {
        ...
        this.$api("order.get", data).then(res=>{
          if(that.$tools.checkResponse(res)){
            that.order = res.data;
            that.order.payToken = that.$store.getters.payToken(res.data.buyerToken)
            resolve(res.data);
          }else{
